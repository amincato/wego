...
export default async function OutgoingApplicationPage({
  params,
}: {
  params: Promise<{ id: string }>;
}) {
  const { id } = await params;
  const app = outgoingApplications.find((a) => a.id === id);
  if (!app) notFound();
  const student = students.find((s) => s.id === app.studentId);
  if (!student) notFound();
  const destinationSchool = schools.find((s) => s.id === app.schoolId);
  ...
  return (
    <>
      <ProfileHeader
        ...
        meta={
          destinationSchool
            ? `Applying to ${destinationSchool.name} · ${destinationSchool.city}, ${destinationSchool.country}`
            : undefined
        }
      />
    ...
    <InfoCard title="Application status">
      ...
      <div className="mt-6 space-y-3 text-sm text-fg-muted">
        <p>
          ...
          The destination school will let us know when they advance the file.
          Current stage:{" "}
        ...
        <div className="flex flex-wrap gap-2">
          <button className="rounded-full bg-surface px-4 py-2 text-xs font-bold text-fg ring-1 ring-divider hover:bg-chip">
            Send reminder to partner school
          </button>
          <button className="rounded-full bg-surface px-4 py-2 text-xs font-bold text-fg ring-1 ring-divider hover:bg-chip">
            Add internal note
          </button>
